...
void plc_status_stop_to_run(void)
{
    ...
    char ret = plc_parse_system_block();
    if (ret == pdPASS)
    {
    ...
    ret = plc_parse_netcfg_block();
    if (ret == pdPASS)
    {
        LOGV(TAG, "plc_parse_netcfg_block SUCCESS!");
    }
    else
    {
        LOGE(TAG, "plc_parse_netcfg_block FAIL!");
    }
    ...
    LOGD(TAG, "g_plc_netcfg.lan.ioExp = %X", g_plc_netcfg.lan.ioExp);
#if (DAISY_MASTER_FEATURE == 1)
    #if (PLC_RUN_WAIT_DAISY == 1)
    if (g_plc_netcfg.lan.ioExp == 0)
    {
        xEventGroupWaitBits(g_kalyke_event_group, KALYKE_EVENT_PLC_TASK_WAIT_DAISY, pdTRUE, pdFALSE, portMAX_DELAY);
    }
    #endif
#endif

#if (KALYKE_MODBUS_TCP_SHEET == 1)
    if (g_plc_netcfg.lan.ioExp == 1)
    {
        start_modbus_tcp();
    }
    start_modbus_com();
