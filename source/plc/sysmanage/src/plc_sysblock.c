...
static uint16_t parse_bus_config(unsigned char *ptr)
{
    ...
    /*
        ...
        1A  01010008 0001        (Maping to 1050）
        0001 
    */
    /* 内存中实际存贮数据
        51 AA 
        ...
        1C  05 10  02  01 00 -- SDO
        1C  06 10  02  00 00 
        
        1B  08 00 02 01  00 00 -- PDO
        1A  08 00 01 01  01 00 
    ...
    */
    ...
    /* 同步数据帧的字节数 */
    ptr += 4;
    gBusConfig.nSynBuffLen = GET_PU16_DATA(ptr);
    LOGI(TAG, "gBusConfig.nSynBuffLen = 0x%X", gBusConfig.nSynBuffLen);

    /* SDO的个数 */
    ptr += 2;
    gBusConfig.nSdoCount = GET_PU16_DATA(ptr);

    /* 同步数据帧pdo的个数 */
    ptr += 2;
    gBusConfig.nPdoCount = GET_PU16_DATA(ptr);
    LOGI(TAG, "gBusConfig.nSdoCount = %u, gBusConfig.nPdoCount = %u", gBusConfig.nSdoCount, gBusConfig.nPdoCount);

    ptr += 2; //Point to : 1C  05 10  02  01 00 -- SDO
    ptr++;    //Point to : 05 10  02  01 00 -- SDO
    /* 解析SDO */
    for (uint16_t i = 0; i < gBusConfig.nSdoCount; i++)
    {
        ...
        case BFM_IDX_0x1005:
            ptr += 3;
            gBusConfig.nBfm1005 = *(ptr + 1) << 8 | *ptr;
            break;

        case BFM_IDX_0x1006:
            ptr += 3;
            gBusConfig.nBfm1006 = *(ptr + 1) << 8 | *ptr;
            break;
    ...
    /* 解析PDO，PDO就是同步数据帧的元素 */
    // 此时ptr已指向: 08 00 02 01  00 00 -- PDO
    ptr--; //此时ptr已指向: 1B 08000201      0000 -- PDO
    ...
    //此时ptr指向： 01 00 -- 有多少分站
    /* 分站的个数，分站的内容不用解析，弄出来直接发给从站，从站解析即可 */
    gBusConfig.nSubStationCount = *(ptr + 1) << 8 | *ptr;
    ptr += 2; //此时指向了分站1的起始处
    LOGI(TAG, "SubStation Count = %u",  gBusConfig.nSubStationCount);
...
*/
    ...
    {
        gBusConfig.item[i].sNum = *(ptr + 1) << 8 | *ptr;
        ...
        LOGV(TAG, "item[%u].len = %u", i, gBusConfig.item[i].len);
...
/**
  ...
  * @retval None
  */
char plc_parse_system_block(void)
{
    unsigned long i;
