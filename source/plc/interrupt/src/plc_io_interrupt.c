...
static void handleCounterValueMinusOnly(uint16_t counterNum)
{
    ...
    if(*pCounterValue == getCounterCompareValue(counterNum))
    {
        plc_set_bit_element_value(C_ELEMENT, HCOUNTER246, 0);
    }
}
...
{
    long *pCounterValue = GET_C32_ADDRESS(counterNum);
    if(GPIO_PinReadPadStatus(X0_GPIO, X0_GPIO_PIN))//如果此时A相为高电平
    {
        setCounterMode(counterNum, true);
        (*pCounterValue)++;
        if(*pCounterValue == getCounterCompareValue(counterNum))
...
void X000_Interrupt(void)
{
    ...
    if (IS_C32_STARTED(HCOUNTER242))
    {
        handleCounterValueAddOrMinus(HCOUNTER242);
    }
    ...
    {
        handleCounterValueAddOnly(HCOUNTER246);
    }
    if (IS_C32_STARTED(HCOUNTER247))
    {
        handleCounterValueAddOnly(HCOUNTER247);
    }
...
void X001_Interrupt(void)
{
    ...
    if (IS_C32_STARTED(HCOUNTER246))
    {
        handleCounterValueMinusOnly(HCOUNTER246);
    }
    if (IS_C32_STARTED(HCOUNTER247))
    {
        handleCounterValueMinusOnly(HCOUNTER247);
    }
    if (IS_C32_STARTED(HCOUNTER249))
    {
    ...
    if (IS_C32_STARTED(HCOUNTER251))
    {
        handleCounterValueBPhase(HCOUNTER251);
    }
    if (IS_C32_STARTED(HCOUNTER252))
    {
        handleCounterValueBPhase(HCOUNTER252);
    }
    if (IS_C32_STARTED(HCOUNTER254))
    {
        handleCounterValueBPhase(HCOUNTER254);
    }
    handleDHSx();
}
...
void X003_Interrupt(void)
{
    ...
    if (IS_C32_STARTED(HCOUNTER243))
    {
        handleCounterValueAddOrMinus(HCOUNTER243);
    }
