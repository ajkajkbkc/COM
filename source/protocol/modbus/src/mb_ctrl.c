...
{
    LOGD("mb_ctrl", "Enter %s(), lcp_Ucode = 0x%08X, llv_Length = %d", __func__, lcp_Ucode, llv_Length);
...
void mb_slave_ctrl_programme_cmd(md_slave_msg_pack *pMsg)
{
    ...
    for(i=0; i<MB_DL_MAX; i++) {
        PRINTF("i = %d, flag = %d\r\n", i, gtv_ModbusFileTrans[i]->mcv_Flag);
        switch(i) {
            case MB_DL_UCODE:
            ...
            case MB_DL_NETCFG:
                ...
                if((NULL != gtv_ModbusFileTrans[i]) && (gtv_ModbusFileTrans[i]->mcv_Flag & 0x02))
                {
                    ...
                    PRINTF("mcp_FileHandler = 0x%08X, xPortGetFreeHeapSize=%u\r\n", gtv_ModbusFileTrans[i]->mcp_FileHandler, xPortGetFreeHeapSize());
                    if (i == MB_DL_SYS_BLOCK)
                    {
                        hexdump(gtv_ModbusFileTrans[i]->mcp_FileHandler, gtv_ModbusFileTrans[i]->mlv_FileLen);
                        vTaskDelay(900);
                    }
                    /*释放动态内存*/
                    vPortFree(gtv_ModbusFileTrans[i]->mcp_FileHandler);
            ...
            case MB_DL_SYS_UPGRADE:
                ...
                if((NULL != gtv_ModbusFileTrans[i]) && (gtv_ModbusFileTrans[i]->mcv_Flag & 0x02))
                {
                    ...
                    {
                        LOGE("mb_ctrl", "gtv_ModbusFileTrans[i]->mlv_FileLen > ltv_FlashPartInfo[i].partSize");
                        /*释放动态内存*/
                        vPortFree(gtv_ModbusFileTrans[i]->mcp_FileHandler);
                        gtv_ModbusFileTrans[i]->mcp_FileHandler = NULL;
                        vPortFree(gtv_ModbusFileTrans[i]);
                    ...
                    LOGD("mb_ctrl", "gSlaveIDUpgrade = %u", gSlaveIDUpgrade);
            ...
            case MB_DL_PLC_CBIN:
                if((NULL != gtv_ModbusFileTrans[i]) && (gtv_ModbusFileTrans[i]->mcv_Flag & 0x02))
                {
                    ...
                    LOGV("mb_ctrl", "UCODE length is %u, partSize = %u", llv_FileLen, ltv_FlashPartInfo[0].partSize);
                    if(llv_FileLen > ltv_FlashPartInfo[0].partSize) {
                        /*释放动态内存*/
                        vPortFree(gtv_ModbusFileTrans[i]->mcp_FileHandler);
                        gtv_ModbusFileTrans[i]->mcp_FileHandler = NULL;
                        vPortFree(gtv_ModbusFileTrans[i]);
                    ...
                    /*喂狗*/
                    bsp_feed_watch_dog();
                    LOGI("mb_ctrl", "xPortGetFreeHeapSize = %u", xPortGetFreeHeapSize());
                    ...
                    /*固化POU*/
                    llv_FileLen = plc_get_file_length(lcp_Buffer, 4);
                    LOGV("mb_ctrl", "POU length is %u", llv_FileLen);
                    ...
                    /*固化GVT*/
                    llv_FileLen = plc_get_file_length(lcp_Buffer, 4);
                    LOGV("mb_ctrl", "GVT length is %u", llv_FileLen);
                    ...
                    /*固化系统块*/
                    llv_FileLen = plc_get_file_length(lcp_Buffer, 4);
                    LOGV("mb_ctrl", "System block length is %u", llv_FileLen);
                    ...
                    /*固化数据块*/
                    llv_FileLen = plc_get_file_length(lcp_Buffer, 4);
                    LOGV("mb_ctrl", "Data block length is %u", llv_FileLen);
...
void mb_slave_ctrl_verify_ucode(md_slave_msg_pack *pMsg)
{
    ...
    PRINTF("llv_FileLen = %d, llv_FileLen = 0x%x\r\n", llv_FileLen, llv_FileLen);
...
static void led_net_blink_task(void *p_arg)
{
    ...
    uint8_t bak1 = GPIO_PinRead(LED_2, LED_2_PIN);
...
{
    LOGV("mb_ctrl", "Enter %s(), tou = %u", __func__, tou);
#if 0
    gTouChuan = 4;
