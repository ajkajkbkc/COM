...
static bool gSOEMInProcess = false;
...
//        curOffset = 0;
        ...
        for (int j = 0; j < gSoemConfig.pSlaves[kalykeSlaveID].pdoTxCount; j++)
        {
            ...
            case DAISY_E_TYPE_M:
            {
                //LOGV(TAG, "gSoemConfig.pSlaves[%d].pTxPDO[%d].eAddr = %u", kalykeSlaveID, j, gSoemConfig.pSlaves[kalykeSlaveID].pTxPDO[j].eAddr);
...
static void kalyke_FR8200_FR2118_SDO_set_PDO_map(int slaveId)
{
...
static void kalyke_LXM28E_SDO_SET_PDO_map(int slaveId)
{
    LOGV(TAG, "Enter %s(), slaveId = %d", __func__, slaveId);
...
RAMFUNCTION_SECTION_CODE(void kalyke_ethercat_task_do(void *p_arg))
{
    ...
    while(1)
    {
        ...
        gWkc = ecx_receive_processdata_group(&ecx_context, 0, EC_TIMEOUTRET);
...
ECCONFIGINIT:
    ...
    for (i = 1; i <= ec_slavecount; i++)
    {
        ...
        if (memcmp(ec_slave[i].name, "FR8200", 6) == 0)
        {
            kalyke_FR8200_FR2118_SDO_set_PDO_map(i);
        }
        else
        {
            kalyke_LXM28E_SDO_SET_PDO_map(i);  
        }
    }
            ...
            for (slave = 1; slave <= ec_slavecount; slave++)
            {
                ...
                if ((ec_slave[slave].group == currentgroup) && (ec_slave[slave].state != EC_STATE_OPERATIONAL))
                {
                    ...
                    if (ec_slave[slave].state == (EC_STATE_SAFE_OP + EC_STATE_ERROR))
                    {
                        ...
                        plc_refresh_error_msg(ERR_EXPANSION_MODULE);
                        guv_NonStopError.bit.ecat_err = 1;
                    ...
                    {
                        LOGW(TAG, "MESSAGE :WARNING : slave %d is in SAFE_OP, change to OPERATIONAL.\n", slave);
