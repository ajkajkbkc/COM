...
extern void kalyke_set_ec_configlist_for_FR8200(ec_slavet *ec_slaveList, int theSlave, ec_configlist_t *pConfigListItem);
...
{/*Man=*/0xE0000002,/*ID=*/0x00000100,/*Name=*/"FR8200(Rev2.0)"  ,/*dtype=*/3,/*Ibits=*/ 0,/*Obits=*/16,/*SM2a*/0x1800,/*SM2f*/      0x64,/*SM3a*/0x1C00,/*SM3f*/      0x20,/*FM0ac*/0,/*FM1ac*/0},
...
#else
ec_configlist_t ec_configlist[3] = {
      {/*Man=*/0x00000000,/*ID=*/0x00000000,/*Name=*/""          ,/*dtype=*/0,/*Ibits=*/ 0,/*Obits=*/ 0,/*SM2a*/     0,/*SM2f*/         0,/*SM3a*/     0,/*SM3f*/         0,/*FM0ac*/0,/*FM1ac*/0},
        
{/*Man=*/0xE0000002,/*ID=*/0x00000100,/*Name=*/"FR8200(Rev2.0)"  ,/*dtype=*/3,/*Ibits=*/ 16,/*Obits=*/16,/*SM2a*/0x1800,/*SM2f*/      0x64,/*SM3a*/0x1C00,/*SM3f*/      0x20,/*FM0ac*/0,/*FM1ac*/0},
...
{
   EC_PRINT("Enter %s(), context = 0x%p", __func__, context);
   uint8  b;
   uint16 w;
   int    wkc;
...
#else
static int kalyke_config_from_configlist_item(ecx_contextt *context, uint16 slave, ec_configlist_t *pConfigListItem)
{
   EC_PRINT("Enter %s(), context = 0x%p, slave = %u, pConfigListItem=0x%p", __func__, context, slave, pConfigListItem);
   int cindex = 1;
   ec_slavet *csl;
   ...
   if (cindex)
   {
      csl->Dtype = pConfigListItem->Dtype;
      strcpy(csl->name ,pConfigListItem->name);
      csl->Ibits = pConfigListItem->Ibits;
      csl->Obits = pConfigListItem->Obits;
      if (csl->Obits)
      {
         csl->FMMU0func = 1;
      }
      ...
      csl->FMMU[0].FMMUactive = pConfigListItem->FM0ac;
      csl->FMMU[1].FMMUactive = pConfigListItem->FM1ac;
      csl->SM[2].StartAddr = htoes(pConfigListItem->SM2a);
      csl->SM[2].SMflags = htoel(pConfigListItem->SM2f);
      /* simple (no mailbox) output slave found ? */
      if (csl->Obits && !csl->SM[2].StartAddr)
      {
         csl->SM[0].StartAddr = htoes(0x0f00);
      ...
      csl->SM[3].StartAddr = htoes(pConfigListItem->SM3a);
      csl->SM[3].SMflags = htoel(pConfigListItem->SM3f);
      /* simple (no mailbox) input slave found ? */
      if (csl->Ibits && !csl->SM[3].StartAddr)
      {
         csl->SM[1].StartAddr = htoes(0x1000);
...
int ecx_config_init(ecx_contextt *context, uint8 usetable)
{
   ...
   EC_PRINT("Enter %s(), usetable = %u", __func__, usetable);
   ecx_init_context(context);
   EC_PRINT("%s : 000000", __func__);
   wkc = ecx_detect_slaves(context);
   EC_PRINT("%s : ecx_detect_slaves return: %d, context->slavecount=%d", __func__, wkc, *context->slavecount);
   if (wkc > 0)
   {
      ...
      for (slave = 1; slave <= *(context->slavecount); slave++)
      {
         ...
         EC_PRINT("In ec_config_init:1111111");
      }
      for (slave = 1; slave <= *(context->slavecount); slave++)
      {
         ...
         EC_PRINT("In ec_config_init:22222222");
      }
      for (slave = 1; slave <= *(context->slavecount); slave++)
      {
         ...
         EC_PRINT("In ec_config_init:333333333");
      }
      for (slave = 1; slave <= *(context->slavecount); slave++)
      {
         ...
         EC_PRINT("In ec_config_init:4444444444");
      }
      for (slave = 1; slave <= *(context->slavecount); slave++)
      {
         ...
         EC_PRINT("In ec_config_init:5555555555");
      }
      for (slave = 1; slave <= *(context->slavecount); slave++)
      {
         if (context->slavelist[slave].mbx_l > 0)
         ...
         {
            #if 1
            EC_PRINT("%s : context->slavelist[%u].name = %s", __func__, slave, context->slavelist[slave].name);
            if (memcmp(context->slavelist[slave].name, "FR8200", 6) == 0)
            {
                ec_configlist_t clItem;
                kalyke_set_ec_configlist_for_FR8200(context->slavelist, slave, &clItem);
                cindex = kalyke_config_from_configlist_item(context, slave, &clItem);
            }
            #else
      ...
      EC_PRINT("In ec_config_init:666666666");
   }
   EC_PRINT("Leave ec_config_init(), wkc = %d", wkc);
   return wkc;
}
...
static int ecx_map_coe_soe(ecx_contextt *context, uint16 slave, int thread_n)
{
   ...
   EC_PRINT("Enter %s, >Slave %d, configadr %x, state %2.2x\n", __func__,
            slave, context->slavelist[slave].configadr, context->slavelist[slave].state);
   EC_PRINT("context->slavelist[%u].PO2SOconfigx = 0x%08X", slave, context->slavelist[slave].PO2SOconfigx);
...
static int ecx_map_sii(ecx_contextt *context, uint16 slave)
{
   ...
   EC_PRINT("Enter %s(), Osize = %d, Isize = %d", __func__, Osize, Isize);
   ...
   uint8 group, int16 slave, uint32 * LogAddr, uint8 * BitPos)
{
   ...
   EC_PRINT("%s:  OUTPUT MAPPING", __func__);
   ...
   {
      EC_PRINT("%s:    FMMU %d", __func__, FMMUc);
      while ((SMc < (EC_MAXSM - 1)) && (context->slavelist[slave].SMtype[SMc] != 3))
      {
         SMc++;
      }
      EC_PRINT("%s:      SM%d", __func__, SMc);
      context->slavelist[slave].FMMU[FMMUc].PhysStart =
         context->slavelist[slave].SM[SMc].StartAddr;
      SMlength = etohs(context->slavelist[slave].SM[SMc].SMlength);
      ByteCount += SMlength;
      ...
      if (!context->slavelist[slave].outputs)
      {
         ...
         context->slavelist[slave].Ostartbit =
            context->slavelist[slave].FMMU[FMMUc].LogStartbit;
         EC_PRINT("%s:    slave %d Outputs %p startbit %d",
            __func__,
            slave,
            context->slavelist[slave].outputs,
            context->slavelist[slave].Ostartbit);
...
int ecx_config_map_group(ecx_contextt *context, void *pIOmap, uint8 group)
{
   ...
   {
      EC_PRINT("%s: IOmap:%p group:%d\n", __func__, pIOmap, group);
      ...
      {
         configadr = context->slavelist[slave].configadr;
         EC_PRINT("%s: configadr = 0x%04X", __func__, configadr);

         if (!group || (group == context->slavelist[slave].group))
         {
            EC_PRINT("%s: context->slavelist[%u].Obits = %u", __func__, slave, context->slavelist[slave].Obits);
