...
char messigeid[32] = {0};
char messigetype[32] = {0};
char messigename[32] = {0};
uint32_t messigeslaveid;
char messigeslavename[32] = {0};
uint16_t messigeslaveadr;
/*******************************************************************************
 * Code
 ******************************************************************************/
...
int8_t Kalyke_extractMqttConfig(const char *pJsonDocument, int32_t tokenCount, mqtt_config_st *pMqttConfig)
{
    ...
    if (pJSONTemp != NULL)
    {
	   ...
	   pJSONTemp = cJSON_GetObjectItemCaseSensitive(json, "subscribe_AlarmSetSD"); // Not necessary
    if (pJSONTemp != NULL)
    {
        if (cJSON_IsString(pJSONTemp) == false)
        {
        ...
        strncpy(pMqttConfig->subscribe_topic_AlarmSetSD, cJSON_GetStringValue(pJSONTemp), sizeof(pMqttConfig->subscribe_topic_AlarmSetSD) - 1);
    }
...
/*
{
  "deviceId" : "24030FID0002CBP2254",
  "timestamp" : 1710757555,
  "headers" : { },
  "messageId" : "111111",
  "data"
  "properties" : {
...
*/
static int8_t Kalyke_extractMqttRecv_NFYMQTT(cJSON *json, mqtt_recv_st *pMqttRecv)
{
    LOGV(TAG, "Enter %s(), Free heap: %d(bytes)", __func__, xPortGetFreeHeapSize());
    ...
    pJSONTemp = cJSON_GetObjectItemCaseSensitive(json, "deviceId");
    if (pJSONTemp == NULL)
    {
        return -11;
    }
    else
    {
        ...
        pMqttRecv->slave_id=atoi(&pMqttRecv->deviceCode[16]);        
        strncpy(pMqttRecv->slave_name, &pMqttRecv->deviceCode[12], 4);
    }

    pJSONTemp = cJSON_GetObjectItemCaseSensitive(json, "timestamp");
    if (pJSONTemp == NULL)
    {
        pMqttRecv->time = 0;
        return -13;
    ...
    pHeaders = cJSON_GetObjectItemCaseSensitive(json, "headers");
    if (pHeaders == NULL)
    {
        LOGE(TAG, "There is no pHeaders keyword!");
        return -15;
    }

    pJSONTemp = cJSON_GetObjectItemCaseSensitive(json, "messageId");
    if (pJSONTemp == NULL)
    {
        return -16;
    }
    else
    {
        ...
        strncpy(pMqttRecv->messageId, cJSON_GetStringValue(pJSONTemp), sizeof(pMqttRecv->messageId) - 1);
        strncpy(messigeid, cJSON_GetStringValue(pJSONTemp), sizeof(pMqttRecv->messageId) - 1);
    }
    
    pJSONTemp = cJSON_GetObjectItemCaseSensitive(json, "messageType");
    if (pJSONTemp == NULL)
    {
        return -18;
    }
    else
    {
        ...
        strncpy(pMqttRecv->messageType, cJSON_GetStringValue(pJSONTemp), sizeof(pMqttRecv->messageType) - 1);
        if ( (strcmp(pMqttRecv->messageType, "INVOKE_FUNCTION") != 0) && (strcmp(pMqttRecv->messageType, "READ_PROPERTY") != 0) && (strcmp(pMqttRecv->messageType, "WRITE_PROPERTY") != 0) )
        {
            return -20;
        }
    }
    
    if (strcmp(pMqttRecv->messageType, "READ_PROPERTY") == 0)
    {
        memcpy(messigetype, "READ_PROPERTY", sizeof("READ_PROPERTY")) ;
        
        pRProperties = cJSON_GetObjectItem(json, "properties");
        if (pRProperties == NULL)
        {
            LOGE(TAG, "There is no pRProperties keyword!");
            return -21;
        }
        
        arraySize = cJSON_GetArraySize(pRProperties);
        pMqttRecv->cmdLength = arraySize;
        pMqttRecv->pCmd = pvPortMalloc(sizeof(mqtt_command_st) * arraySize);
        memset(pMqttRecv->pCmd, 0, sizeof(mqtt_command_st) * arraySize);
        pCmd = pMqttRecv->pCmd;
        ...
        if (pRProperties && cJSON_IsArray(pRProperties)) 
        {
            int i;
            for (i = 0; i < cJSON_GetArraySize(pRProperties); i++) 
            {
                cJSON *item = cJSON_GetArrayItem(pRProperties, i);
                if (item && cJSON_IsString(item)) 
                {
                    LOGE("MQTT", "messigename: %s\n", item->valuestring);
                    strncpy(pCmd[i].name, item->valuestring, sizeof(pCmd[i].name) - 1);
                    memcpy(messigename, pCmd[i].name, sizeof(messigename)) ;
                }
            }
        }
    }
    
    else if(strcmp(pMqttRecv->messageType, "WRITE_PROPERTY") == 0)
    {
        memcpy(messigetype, "WRITE_PROPERTY", sizeof("WRITE_PROPERTY")) ;
        
        pWProperties = cJSON_GetObjectItemCaseSensitive(json, "properties");
        
        cJSON *node = NULL;
        node = pWProperties->child;
        
        while (node != NULL)
        {
            LOGE("MQTT","%s\n",node->string);
            strncpy(pMqttRecv->WPROPERTY, node->string, sizeof(pMqttRecv->WPROPERTY) - 1);
            LOGE(TAG, "pCmd[0].name:%s", pMqttRecv->WPROPERTY);
            node = node->next;
        }

        if (pWProperties == NULL)
        {
            LOGE(TAG, "There is no inputs keyword!");
            return -22;
        }
        else
        {
            arraySize = cJSON_GetArraySize(pWProperties);
            pMqttRecv->cmdLength = arraySize;
            pMqttRecv->pCmd = pvPortMalloc(sizeof(mqtt_command_st) * arraySize);
            memset(pMqttRecv->pCmd, 0, sizeof(mqtt_command_st) * arraySize);
            pCmd = pMqttRecv->pCmd;
                
            pJSONTemp = cJSON_GetObjectItemCaseSensitive(pWProperties, pMqttRecv->WPROPERTY);
            if (pJSONTemp == NULL)
            {
                return -23;
            }
            ...
            {
                strncpy(pCmd[0].name, pMqttRecv->WPROPERTY, sizeof(pCmd[0].name) - 1);
                LOGI(TAG, "pCmd[0].name:%s", pCmd[0].name);
                memcpy(messigename, pMqttRecv->WPROPERTY, sizeof(pMqttRecv->WPROPERTY)) ;
                if (cJSON_IsNumber(pJSONTemp) == false)
                {
                    return -24;
                }
                ...
                {
                    pCmd[0].valueFloat = cJSON_GetNumberValue(pJSONTemp);
                    pCmd[0].valueInt32 = cJSON_GetNumberValue(pJSONTemp);
                    pCmd[0].valueInt16 = cJSON_GetNumberValue(pJSONTemp);
                    pCmd[0].valueBool = pCmd[0].valueInt16;                
                }
            }
        }
    }
    
    else if (strcmp(pMqttRecv->messageType, "INVOKE_FUNCTION") == 0)
    {
        memcpy(messigetype, "INVOKE_FUNCTION", sizeof("INVOKE_FUNCTION")) ;
        
        pDatas = cJSON_GetObjectItemCaseSensitive(json, "inputs");
        if (pDatas == NULL)
        {
            LOGE(TAG, "There is no inputs keyword!");
            return -25;
        ...
        else
        {
            ...
            cJSON_ArrayForEach(pAData, pDatas)
            {
                ...
                {
                    LOGE(TAG, "value is a string.");
                    
                    if(cJSON_IsBool(pJSONTemp) && (pJSONTemp->type == cJSON_True))
                    {   
                        pCmd[j].valueFloat = 1.000000;
                        pCmd[j].valueInt32 = 1;
                        pCmd[j].valueInt16 = 1;
                        pCmd[j].valueBool = 1;
...
int8_t Kalyke_extractMqttRecv(const char *pJsonDocument, int32_t tokenCount, mqtt_recv_st *pMqttRecv)
{
    ...
    {
        retVal = Kalyke_extractMqttRecv_NFYMQTT(json, pMqttRecv);
        goto ENDME;
    }
