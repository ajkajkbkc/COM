...
static uint8_t get_WeekDayNum(uint32_t nYear, uint8_t nMonth, uint8_t nDay)
{
...
static void kalyke_second_task(void *p_arg)
{
    ...
    LOGW("Kalyke_monitor", "kalyke_second_task RUN. Free heap size is %d bytes\r\n", xPortGetFreeHeapSize());
    SNVS_HP_RTC_GetDatetime(SNVS, &rtcDate);
    SET_SD_ELEMENT_VALUE(100, rtcDate.year);
    SET_SD_ELEMENT_VALUE(101, rtcDate.month);
    SET_SD_ELEMENT_VALUE(102, rtcDate.day);
...
static uint8_t getElementValue_bool(mqtt_config_array_st *pCfg)
{
    ...
    else if (pCfg->element == ELEM_SM) //(memcmp(pCfg->element, "SM", 2) == 0)
    {
...
static uint16_t getElementValue_uint16(mqtt_config_array_st *pCfg)
{
    uint16_t low16 = GET_D_ELEMENT_VALUE(pCfg->address);
    uint16_t high16 = GET_D_ELEMENT_VALUE(pCfg->address+1);
    uint16_t val = (low16 << 0) | high16;
    return val;
}

static uint32_t getElementValue_uint32(mqtt_config_array_st *pCfg)
{
    uint16_t low16 = GET_D_ELEMENT_VALUE(pCfg->address);
    uint16_t high16 = GET_D_ELEMENT_VALUE(pCfg->address+1);
    uint32_t val = (low16 << 8) | high16;
    return val;
}
...
* @retval       非0 失败，以及失败原因
*/
long long fml_time_to_stamp(snvs_hp_rtc_datetime_t date)
{
    ...
    dax = (uint32_t)(day_count * 86400) + (uint32_t)((uint32_t)date.hour * 3600) + (uint32_t)((uint32_t)date.minute * 60) + (uint32_t)date.second;
 
    milliseconds = (long long)dax*1000;
    /* 北京时间补偿 */
    milliseconds = milliseconds - 8*60*60*1000;
...
//            sprintf(tempStr, "\"device_id\":\"%s\",\"slave_id\":%d,", device_id,pCfgs->slave_id);
    ...
    else if (memcmp(g_plc_netcfg.mqtt.vender, "NFYMQTT", 7) == 0)
    {
        ...
        sprintf(tempStr, "\"deviceId\":\"%s%s%d\",", device_id, pCfgs->slave_name, pCfgs->slave_id);
        strcat(mqttBuf, tempStr);
        
        long long ms = 0;
        ms = fml_time_to_stamp(rtcDate);
        
        sprintf(tempStr, "\"timestamp\":\%lld,", ms);
        strcat(mqttBuf, tempStr);
    ...
    else if (memcmp(g_plc_netcfg.mqtt.vender, "SUZHOUMQTT", 10) == 0)
    {
        mqtt_config_array_st *pRC = pCfgs->pReportContent;

        for(int i = 0; i < pCfgs->reportContentLen; i++)  //斯訍式戮莸愣际嘢YM_CFG 圆虏禄楔要蛦藣
        {
            if(pRC[i].sym != SYM_CFG)
            {
                break;
            }
            if(i >= pCfgs->reportContentLen - 1) //直碌陆桩潞髵桓枚式戮莸愣际嘢YM_CFG
            {
                vPortFree(tempStr);
                vPortFree(mqttBuf);
                LOGD("Kalyke_monitor", "Leave %s(), mqttBuf = %s", __func__, mqttBuf);
        ...
        {        
            sprintf(tempStr, "\"dnygfdl\":\"%d\",", getElementValue_uint32(&pRC[3]));
            strcat(mqttBuf, tempStr);
        }
        else
        {        
        ...
        if(0 ==memcmp("GCI1", pCfgs->slave_name, 4))
        {        
            sprintf(tempStr, "\"drygfdl\":\"%d\",", getElementValue_uint32(&pRC[2]));
            strcat(mqttBuf, tempStr);
        }
        else if(0 ==memcmp("GWDT", pCfgs->slave_name, 4))
        {        
            sprintf(tempStr, "\"drygfdl\":\"%d\",", getElementValue_uint16(&pRC[1]));
            strcat(mqttBuf, tempStr);
        }
        ...
        {        
            sprintf(tempStr, "\"drygfdl\":\"%d\",", getElementValue_uint32(&pRC[1]));
            strcat(mqttBuf, tempStr);
        }
        ...
        if(0 ==memcmp("GCI1", pCfgs->slave_name, 4))
        {        
            sprintf(tempStr, "\"dyygfdl\":\"%d\",", getElementValue_uint32(&pRC[1]));
            strcat(mqttBuf, tempStr);
        }
        else
        {        
        ...
        char device_topic[CONFIG_MQTT_MAX_LWT_TOPIC];
        sprintf(device_topic, "FEXCLOUD/%s/DATAPUB/RPT/%s", g_plc_netcfg.mqtt.vender, device_id);
...
//            sprintf(tempStr, "\"device_id\":\"%s\",\"slave_id\":%d,", device_id,pCfgs->slave_id);
        ...
        for (int i = 0; i < pCfgs->reportContentLen; i++)
        {
            ...
            {
                sprintf(tempStr, "{\"name\":\"%s\",\"value\":%s,\"cont\":\"%s\"},", pRC[i].name, valStr, pRC[i].alarmContent);  //显示触发content
                pRC[i].alarmType = true;
            }
            else
            {
                sprintf(tempStr, "{\"name\":\"%s\",\"value\":%s,\"cont\":\"\"},", pRC[i].name, valStr);  //不显示触发content
    ...
    {   
        ...
        sprintf(tempStr, "\"messageId\":\"%s\",", messigeid);
        strcat(mqttBuf, tempStr);
        
        sprintf(tempStr, "\"output\":\"%s\",", "success");
        strcat(mqttBuf, tempStr);
    
        sprintf(tempStr, "\"success\":\%s,", "true");
        strcat(mqttBuf, tempStr);
...
//        strcat(mqttBuf, tempStr);
//    
//        sprintf(tempStr, "\"success\":\%s,", alarmType);
//        strcat(mqttBuf, tempStr);
        ...
        memcpy(device_id, gtv_DeviceConfigTable.mtv_DevInfo.mcv_DeviceId, 12);
        char device_topic[CONFIG_MQTT_MAX_LWT_TOPIC];
        sprintf(device_topic, "/%s/%s%s%d/function/invoke/reply", device_id, device_id, pCfgs->slave_name, pCfgs->slave_id);
        kalyke_mqtt_publish(device_topic, mqttBuf, 0);
    }
...
static void kalyke_alarm_post_nfymqtt(mqtt_config_array_hanyu_st *pCfgs, char eventRecover)
{
    LOGV("Kalyke_monitor", "Enter %s(), reportContentLen = %u", __func__, pCfgs->reportContentLen);
    char *tempStr = pvPortMalloc(1024);
    char *mqttBuf = pvPortMalloc(4096);
    ...
    if(memcmp(g_plc_netcfg.mqtt.vender, "NFYMQTT", 7) == 0)
    {   
        char device_id[24] = {0};
        memcpy(device_id, gtv_DeviceConfigTable.mtv_DevInfo.mcv_DeviceId, 12);
        ...
        if(strcmp(messigetype, "READ_PROPERTY") == 0)
        {
            LOGE("READ_PROPERTY", "pCfgs->slave_id: %d\n", pCfgs->slave_id);
            LOGE("READ_PROPERTY", "pCfgs->slave_name: %s\n", pCfgs->slave_name);
            
//            pCfgs->slave_id = messigeslaveid;
//            strncpy(pCfgs->slave_name, messigeslavename, 4);
            
            if(pCfgs->slave_id == messigeslaveid && strcmp(pCfgs->slave_name, messigeslavename) == 0)
            {                
                mqtt_config_array_st *pRC = pCfgs->pReportContent;
                strcat(mqttBuf, "\"properties\":{");
                for (int i = 0; i < pCfgs->reportContentLen; i++)
                {                    
                    ...
                    //if(strcmp(pRC[i].name, messigename) == 0 && pRC[i].address == messigeslaveadr) 
                    if(strcmp(pRC[i].name, messigename) == 0) 
                    {
                        if (pRC[i].dataType == DTYPE_F32)  //(strcmp(pRC[i].dataType, "float32") == 0)
                        {
                            sprintf(tempStr, "\"%s\":\"%.3f\"", pRC[i].name, getElementValue_float(&pRC[i]));
...
//            strcat(mqttBuf, "\"properties\":{");
//            
//            sprintf(tempStr, "\"%s\":\"%s\"", messigename, "1");
//            
//            strcat(mqttBuf, tempStr);
//            strcat(mqttBuf, "},");
//            
            ...
            {
                sprintf(tempStr, "\"success\":\%s,", "false");
                strcat(mqttBuf, tempStr);
                
                sprintf(tempStr, "\"code\":\"%s\",", "error_code");
                strcat(mqttBuf, tempStr);
                
                sprintf(tempStr, "\"message\":\"%s\"", "fail");
                strcat(mqttBuf, tempStr);
            }
...
//                mqttBuf[len - 1] = '\0';
            ...
            char device_topic[CONFIG_MQTT_MAX_LWT_TOPIC];
            sprintf(device_topic, "/%s/%s%s%d/properties/read/reply", device_id, device_id, pCfgs->slave_name, pCfgs->slave_id);
            kalyke_mqtt_publish(device_topic, mqttBuf, 0);            
        }
        
        else if(strcmp(messigetype, "WRITE_PROPERTY") == 0)
        {
            mqtt_config_array_st *pRC = pCfgs->pReportContent;
            strcat(mqttBuf, "\"properties\":{");
            for (int i = 0; i < pCfgs->reportContentLen; i++)
...
//                mqttBuf[len - 1] = '\0';
            ...
            char device_topic[CONFIG_MQTT_MAX_LWT_TOPIC];
            //sprintf(device_topic, "/%s/%s%s%d/properties/write/reply", device_id, device_id, messigeslavename, messigeslaveid);
            sprintf(device_topic, "/%s/%s%s%d/properties/write/reply", device_id, device_id, pCfgs->slave_name, pCfgs->slave_id);
            kalyke_mqtt_publish(device_topic, mqttBuf, 0);
        }
        
        else if(strcmp(messigetype, "INVOKE_FUNCTION") == 0)
        {
            sprintf(tempStr, "\"output\":\"%s\",", "success");
            strcat(mqttBuf, tempStr);
        ...
        if (strcmp(g_plc_netcfg.mqtt.vender, "NFYMQTT") == 0)
        {
            for (int i = 0; i < g_plc_netcfg.mqtt.configLength; i++)
            {
                ...
                {                
                    if(strcmp(messigetype, "READ_PROPERTY") == 0)
                    {
                        if(g_plc_netcfg.mqtt.pConfigsHANYU[i].slave_id == messigeslaveid && strcmp(g_plc_netcfg.mqtt.pConfigsHANYU[i].slave_name, messigeslavename) == 0 && pRC[j].address == messigeslaveadr)
                        {
                            kalyke_alarm_post_nfymqtt(&g_plc_netcfg.mqtt.pConfigsHANYU[i], eventRecover);
                            memset(messigetype, 0, sizeof(messigetype));
                            break;
                        }
                    }

                    else if(strcmp(messigetype, "WRITE_PROPERTY") == 0 || strcmp(messigetype, "INVOKE_FUNCTION") == 0)
                    {
                        if(g_plc_netcfg.mqtt.pConfigsHANYU[i].slave_id == messigeslaveid && strcmp(g_plc_netcfg.mqtt.pConfigsHANYU[i].slave_name, messigeslavename) == 0)
                        {
