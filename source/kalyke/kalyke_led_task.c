...
void led_task(void *p_arg)
{
    ...
    static uint32_t flll = 0;
    LOGV("led_task", "led_task RUN. Free heap size is %d bytes\r\n", xPortGetFreeHeapSize());
#endif
    if(gtv_DeviceConfigTable.mcv_IsSupportPlsd == 0)
    {
    ...
    LOGV("led_task", "%s : KALYKE_EVENT_ENET_INIT_DONE_LED", __func__);
...
/*
        ...
        {
            #if 1
            if (guv_NonStopError.bit.ecat_slave_num_err)
            {
                LED_RUN->DR_SET = LED_1_PIN_MASK;
                bsp_toggle_led_ERR();
                goto DELAY_800MS;
