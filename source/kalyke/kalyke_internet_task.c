...
static struct netif fsl_netif0; // For WAN
...
static phy_handle_t phyHandle2   = {.phyAddr = PHY_ADDRESS_2, .mdioHandle = &mdioHandle2, .ops = &phyksz8081_ops};
...
static void tcpSendTimeoutCB(TimerHandle_t ltv_TimeHandle)
{
    ...
    LOGE("internet", "Enter %s(), clientID = %u", __func__, clientID);
...
void mbc_link_send(uint8_t *pBuff, uint16_t len, uint8_t clientID, uint8_t *ucode_pc, uint8_t flag, uint16_t *registerBuf)
{
    //kalyke_set_LAN_default();
    LOGV("internet", "Enter %s(), clientID = %u", __func__, clientID);
    gMBCLinkSend[clientID].pc = ucode_pc;
    gMBCLinkSend[clientID].flag = flag;
    gMBCLinkSend[clientID].resultData = (uint8_t *)registerBuf;
    if (len > 128)
    {
        hexdump(pBuff, 128);
    }
    ...
    start_tcp_send_timeout_timer(clientID);
}
...
{
    uint16_t ti = gKalykeSecondTick;
    LOGE("MicroLinkClient", "Enter %s()", __func__);
    ...
    while(1)
    {
        ...
        if (g_connClient[clientID] != NULL)
        {
            ...
            if(err == ERR_OK)
            {
                ...
                netbuf_delete(buf);
                LOGE("MicroLinkClient", "Disconnected from server: %s:%u !!! err = %d", ipaddr_ntoa(&ipTarget), portTarget, err);
                MicroLink_StopHeart();
...
static void tcpServerThread(void *arg)
{
    ...
    LOGV("tcpServerThread", "Enter %s()", __func__);
...
LISTEN_ME_AGAIN:
    ...
    if (readDialSwitch2() == 1)//拨码开关2置于ON了
    {
        IP4_ADDR(&address, 192U, 168U, 0U, 10U);
    }
    else
    {
    ...
    if (g_conn == NULL)
    {
        ...
        LOGE("tcpServerThread", "conn == NULL, so just return....Line:%d", __LINE__);
        vTaskDelete(NULL);
        return;
    }
    ...
    LOGV("tcpServerThread", "Line:%d, listen on netconn : 0x%08X", __LINE__, g_conn);
    
    while (1)
    {
        /* Grab new connection. */
        LOGW("tcpServerThread", "Wait remote to connect ME: %s:%u", ipaddr_ntoa(&address), 502);
        err = netconn_accept(g_conn, &g_connServer);
        LOGV("tcpServerThread", "netconn_accept return: %d", err);
        netconn_peer(g_connServer, &address, &port);
        LOGV("tcpServerThread", "peer IPv4 Address  : %s,  port: %u\r\n", ipaddr_ntoa(&address), port);
        netconn_addr(g_connServer, &address, &port);
        LOGW("tcpServerThread", "local IPv4 Address : %s,  port: %u\r\n", ipaddr_ntoa(&address), port);
    #if 0
        netconn_set_nonblocking(g_connServer, 1);
        uint8_t blocking = netconn_is_nonblocking(g_connServer);
        LOGW("tcpServerThread", "blocking = %u", blocking);
    #endif
        ...
        if (err == ERR_OK)
        {
        ...
        #endif
            LOGV("tcpServerThread", "accepted new connection : 0x%08X", g_connServer);
            struct netbuf *buf;
            void *data;
            u16_t len;
            ...
            LOGE("tcpServerThread", "netconn_recv return: %d", err);
            SET_SD_ELEMENT_VALUE(SD260, err);
        ...
        #if (TCP_SERVER_LOGIC_2 == 1)
            gTcpServerCont--;
            LOGW("tcpServerThread", "gTcpServerCont = %d", gTcpServerCont);
            if (gTcpServerCont == 1)
            {
                netconn_delete(g_conn);
                goto LISTEN_ME_AGAIN;
            }
        #endif
        }
    }
...
static void tcpServerThread2(void *arg)
{
    ...
    LOGV("tcpServerThread2", "Enter %s()", __func__);
    ...
    if (g_conn == NULL)
    {
        LOGE("tcpServerThread2", "conn == NULL, so just return.");
    ...
    /* Tell connection to go into listening mode. */
    //netconn_listen(conn);
    //LOGV("tcpServerThread2", "listen on netconn : 0x%08X", conn);
    while (1)
    {
        /* Grab new connection. */
        LOGW("tcpServerThread2", "Wait remote to connect ME!!!");
        err = netconn_accept(g_conn, &g_connServer2);
        LOGV("tcpServerThread2", "netconn_accept return: %d", err);
        netconn_peer(g_connServer2, &addr, &port);
        LOGV("tcpServerThread2", "peer IPv4 Address  : %s,  port: %u\r\n", ipaddr_ntoa(&addr), port);
        netconn_addr(g_connServer2, &addr, &port);
        LOGW("tcpServerThread2", "IPv4 Address  : %s,  port: %u\r\n", ipaddr_ntoa(&addr), port);
        ...
        if (err == ERR_OK)
        {
        ...
        #endif
            LOGV("tcpServerThread2", "accepted new connection : 0x%08X", g_connServer2);
            struct netbuf *buf;
            void *data;
            u16_t len;
            ...
            LOGE("tcpServerThread2", "netconn_recv return: %d", err);
            SET_SD_ELEMENT_VALUE(SD261, err);
        ...
        #if (TCP_SERVER_LOGIC_2 == 1)
            gTcpServerCont--;
            LOGW("tcpServerThread2", "gTcpServerCont = %d", gTcpServerCont);
            if (gTcpServerCont == 1)
            {
                netconn_delete(g_conn);
                g_conn = netconn_new(NETCONN_TCP);
                LOGV("tcpServerThread2", "listen on netconn : 0x%08X", g_conn);
                if (g_conn == NULL)
                {
                    LOGE("tcpServerThread2", "g_conn == NULL!!!!!.....Line: %d", __LINE__);
                }
                addr.addr = g_plc_netcfg.wan.ip.addr;
                netconn_bind(g_conn, &addr, 502);
                netconn_listen_with_backlog(g_conn, 1);
            }
        #endif
        }
...
static void tcpServerThread3(void *arg)
{
    ...
    /* Tell connection to go into listening mode. */
    //netconn_listen(conn);
    //LOGV(TAGTCPS3, "listen on netconn : 0x%08X", conn);
    while (1)
    {
        /* Grab new connection. */
...
static void enet2_tcpServerThread(void *arg)
{
    ...
    LOGV("enet2_tcpServerThread", "Enter %s()", __func__);
    /* Create a new connection identifier. */
    /* Bind connection to well known port number 502. */
#if LWIP_IPV6
    ...
    if (readDialSwitch2() == 1)//拨码开关2置于ON了
    {
        IP4_ADDR(&address, 192U, 168U, 0U, 11U);
    }
    else
    {
    ...
    if (g_connEnet2 == NULL)
    {
        LOGE("enet2_tcpServerThread", "g_connEnet2 == NULL, so just return.");
    ...
    LOGV("enet2_tcpServerThread", "listen on netconn : 0x%08X", g_connEnet2);
    while (1)
    {
        /* Grab new connection. */
        LOGW("enet2_tcpServerThread", "Wait remote to connect ME: %s:%u", ipaddr_ntoa(&address), 502);
        err = netconn_accept(g_connEnet2, &g_connEnet2Server);
        LOGV("enet2_tcpServerThread", "netconn_accept return: %d", err);
        netconn_peer(g_connEnet2Server, &address, &port);
        LOGV("enet2_tcpServerThread", "peer IPv4 Address  : %s,  port: %u\r\n", ipaddr_ntoa(&address), port);
        netconn_addr(g_connEnet2Server, &address, &port);
        LOGW("enet2_tcpServerThread", "local IPv4 Address : %s,  port: %u\r\n", ipaddr_ntoa(&address), port);
        SET_SD_ELEMENT_VALUE(SD263, err);
			/* Process the new connection. */
        if (err == ERR_OK)
        {
            LOGV("enet2_tcpServerThread", "accepted new connection : 0x%08X", g_connEnet2Server);
            struct netbuf *buf;
            void *data;
            u16_t len;
...
static void enet2_tcpServerThread2(void *arg)
{
    ...
    LOGV("enet2_tcpServerThread2", "Enter %s()", __func__);
    vTaskDelay(1001 / portTICK_PERIOD_MS);

    if (g_connEnet2 == NULL)
    {
        LOGE("enet2_tcpServerThread2", "g_connEnet2 == NULL, so just return.");
    ...
    {
        /* Grab new connection. */
        LOGW("enet2_tcpServerThread2", "Wait remote to connect ME!!!");
        err = netconn_accept(g_connEnet2, &g_connEnet2Server2);
        LOGV("enet2_tcpServerThread2", "netconn_accept return: %d", err);
        netconn_peer(g_connEnet2Server2, &addr, &port);
        LOGV("enet2_tcpServerThread2", "peer IPv4 Address  : %s,  port: %u\r\n", ipaddr_ntoa(&addr), port);
        netconn_addr(g_connEnet2Server2, &addr, &port);
        LOGW("enet2_tcpServerThread2", "IPv4 Address  : %s,  port: %u\r\n", ipaddr_ntoa(&addr), port);
        /* Process the new connection. */
        SET_SD_ELEMENT_VALUE(SD264, err);
        if (err == ERR_OK)
        {
            LOGV("enet2_tcpServerThread2", "accepted new connection : 0x%08X", g_connEnet2Server2);
            struct netbuf *buf;
            void *data;
            u16_t len;
...
void start_tcp_server(void)
{
    ...
    BaseType_t ret = xTaskCreate((TaskFunction_t)tcpServerThread,
                                 ...
                                 WAN_TCPSERVER_TASK_STACK_SIZE,
                                 (void *)NULL,
                                 WAN_TCPSERVER_TASK_PRIO,
...
//        char sub_topic_read[CONFIG_MQTT_MAX_LWT_TOPIC];
//        char sub_topic_write[CONFIG_MQTT_MAX_LWT_TOPIC];
//        char sub_topic_invoke[CONFIG_MQTT_MAX_LWT_TOPIC];
//        
//        for (int i = 0; i < g_plc_netcfg.mqtt.configLength; i++)
//        {
//            sprintf(sub_topic_read, "/%s/%s%s%d/properties/read",device_id, device_id, g_plc_netcfg.mqtt.pConfigsHANYU[i].slave_name, g_plc_netcfg.mqtt.pConfigsHANYU[i].slave_id);
//            //mqtt_QMTSUB_do(sub_topic);  
//            mqtt_sub_unsub(client,
//               sub_topic_read, 0,
//               mqtt_topic_subscribed_cb, LWIP_CONST_CAST(void *, sub_topic_read),
//               1);
//            LOGI("read", "subscribe_topic %s", sub_topic_read);
//            
//            sprintf(sub_topic_write, "/%s/%s%s%d/properties/write",device_id, device_id, g_plc_netcfg.mqtt.pConfigsHANYU[i].slave_name, g_plc_netcfg.mqtt.pConfigsHANYU[i].slave_id);
//            //mqtt_QMTSUB_do(sub_topic);
//            mqtt_sub_unsub(client,
//               sub_topic_write, 0,
//               mqtt_topic_subscribed_cb, LWIP_CONST_CAST(void *, sub_topic_write),
//               1);
//            LOGD("write", "subscribe_topic %s", sub_topic_write);
//            
//            sprintf(sub_topic_invoke, "/%s/%s%s%d/function/invoke",device_id, device_id, g_plc_netcfg.mqtt.pConfigsHANYU[i].slave_name, g_plc_netcfg.mqtt.pConfigsHANYU[i].slave_id);
//            //mqtt_QMTSUB_do(sub_topic);
//            mqtt_sub_unsub(client,
//               sub_topic_invoke, 0,
//               mqtt_topic_subscribed_cb, LWIP_CONST_CAST(void *, sub_topic_invoke),
//               1);
//            LOGE("invoke", "subscribe_topic %s", sub_topic_invoke);
//        }
//    }
    ...
    if (memcmp(g_plc_netcfg.mqtt.vender, "HANYU", 5) == 0 || memcmp(g_plc_netcfg.mqtt.vender, "DEFAULTMQTT", 11) == 0 )
    {
        ...
        if (strlen(g_plc_netcfg.mqtt.subscribe_topic_AlarmSetSD) != 0)
        {
            mqtt_sub_unsub(client,
                       g_plc_netcfg.mqtt.subscribe_topic_AlarmSetSD, 0,
                       mqtt_topic_subscribed_cb, LWIP_CONST_CAST(void *, g_plc_netcfg.mqtt.subscribe_topic_AlarmSetSD),
                       1);
        }
...
static void mqtt_init(void)
{
    ...
    while (1)
    {
        if (!gWanOK)
        {
            LOGE("MQTT", "Just waitting, because there is no WAN now.");
            vTaskDelay(10000);
        }
        else
        {
...
static void handle_mqtt_recv_data_non_json(mqtt_msg_st *pMqttMsg)
{
    ...
    //(void)len;
    if (strcmp(g_plc_netcfg.mqtt.vender, "HANYU") == 0 || strcmp(g_plc_netcfg.mqtt.vender, "DEFAULTMQTT") == 0 || strcmp(g_plc_netcfg.mqtt.vender, "SUZHOUMQTT") == 0  )
    {
    ...
    else if (strncmp(msg, "inituart4", 9) == 0)
    {
        init_UART4();
    }
...
static void process_mqtt_recv_data_NFYMQTT(mqtt_recv_st *pmr)
{
    LOGV("MQTT", "Enter %s()", __func__);

    LOGI("MQTT", "pmr->deviceCode:%s", pmr->deviceCode);
    
    LOGI("MQTT", "pmr->slave_id:%d", pmr->slave_id);
    messigeslaveid = pmr->slave_id;
    LOGE("MQTT", "messigeslaveid: %d\n", messigeslaveid);
    
    LOGI("MQTT", "pmr->slave_name:%s", pmr->slave_name);
    strncpy(messigeslavename, pmr->slave_name, 4);
    LOGE("MQTT", "messigeslavename: %s\n", messigeslavename);
    
    LOGI("MQTT", "pmr->time:%lld", pmr->time);
    LOGI("MQTT", "pmr->messageId:%s", pmr->messageId);
    LOGI("MQTT", "pmr->cmdLength:%d", pmr->cmdLength);
    LOGI("MQTT", "pmr->RPROPERTY:%s", pmr->RPROPERTY);
    LOGI("MQTT", "pmr->WPROPERTY:%d", pmr->WPROPERTY);

    if (pmr->cmdLength > 0)
    {
        LOGI("MQTT", "pmr->cb_swicth:%s, %d", pmr->pCmd[0].name, pmr->pCmd[0].valueInt32);
        LOGI("MQTT", "pmr->cb_enable:%s, %d", pmr->pCmd[1].name, pmr->pCmd[1].valueInt32);
    }

//    if(strcmp(messigetype, "WRITE_PROPERTY") == 0 || strcmp(messigetype, "INVOKE_FUNCTION") == 0)
//    {
        for (int j = 0; j < g_plc_netcfg.mqtt.configLength; j++) //分组循环
        {
            for (int i = 0; i < pmr->cmdLength; i++)
...
//                        LOGI("MQTT", "messigeslaveid: %d\n", messigeslaveadr);
                    ...
                    else if(strcmp(messigetype, "READ_PROPERTY") == 0)
                    {
                        messigeslaveadr = adr;
                        LOGI("MQTT", "messigeslaveid: %d\n", messigeslaveadr);
                        //break;
...
void handle_mqtt_recv_data(mqtt_msg_st *pMqttMsg)
{
    ...
    //阳江风电塔项目要求收到alarm/rpt的报文后，能报警
    char *mqttTopic = (char *)pMqttMsg->topic;
    if ((strcmp(g_plc_netcfg.mqtt.vender, "HANYU") == 0 || strcmp(g_plc_netcfg.mqtt.vender, "DEFAULTMQTT") == 0 )
        && strstr(mqttTopic, "ALARM/RPT"))
    {
        LOGI("MQTT", "finding Alarm/rpt, and SET_SD_ELEMENT_VALUE(SD402, 1)"); 
        SET_SD_ELEMENT_VALUE(SD402, 1);
        return;
    }
    int tokenCount = 0;
    mqtt_recv_st mr = {0};
    ...
    else
    {
        ...
        else if (strcmp(g_plc_netcfg.mqtt.vender, "NFYMQTT") == 0 )
        {
            process_mqtt_recv_data_NFYMQTT(&mr);
            //monitor_publish_now();
        }
...
void mqtt_thread(void *arg)
{
    ...
    if (g_plc_netcfg.wan.notUseDHCP == 1)
    {
        goto GOT_IP;
    }
...
GOT_IP:
    if (readDialSwitch2() || g_plc_netcfg.wan.isParsed != 1)
    {
        ip_addr_t dnsIP;
...
void init_internet_ENET(void)
{
    ...
    uint8_t dialSwitch = readDialSwitch2();
    LOGV("ENET1", "dialSwitch = %u, notUseDHCP = %u", dialSwitch, g_plc_netcfg.wan.notUseDHCP);

    if (dialSwitch == 1)//拨码开关2置于ON了
    {
    ...
    else if (g_plc_netcfg.wan.isParsed != 1)
    {
        IP4_ADDR(&fsl_netif0_ipaddr, 192U, 168U, 0U, 10U);
        IP4_ADDR(&fsl_netif0_netmask, 255U, 255U, 255U, 0U);
        IP4_ADDR(&fsl_netif0_gw, 192U, 168U, 0U, 1U);
    }
    else if (g_plc_netcfg.wan.notUseDHCP == 1)
    {
    ...
    if (readDialSwitch2() == 0)//拨码开关2置于OFF
    {
...
void init_internet_ENET2(void)
{
    ...
    if (readDialSwitch2() == 1)//拨码开关2置于ON了
    {
        IP4_ADDR(&fsl_netif1_ipaddr, 192U, 168U, 0U, 11U);
        IP4_ADDR(&fsl_netif1_netmask, 255U, 255U, 255U, 0U);
        IP4_ADDR(&fsl_netif1_gw, 192U, 168U, 0U, 1U);
    }
    else if (g_plc_netcfg.lan.ioExp == 1)
    {
        fsl_netif1_ipaddr.addr = g_plc_netcfg.lan.ip.addr;
        fsl_netif1_netmask.addr = g_plc_netcfg.lan.mask.addr;
        fsl_netif1_gw.addr = g_plc_netcfg.lan.gate.addr;
    }
    else if (g_plc_netcfg.lan.ioExp == 0)
    {
    ...
    else if ((g_plc_netcfg.lan.ioExp == 0) || (g_plc_netcfg.lan.ioExp == 2))
    {
        ret = netifapi_netif_add(&fsl_netif1, &fsl_netif1_ipaddr, &fsl_netif1_netmask, &fsl_netif1_gw, &fsl_enet_config1,
                             ethernetif1_init, ethernet_input);
    }
    ...
    else if (g_plc_netcfg.lan.ioExp == 2)  //EtherCAT
    {
        kalyke_start_EtherCAT();
    }
    else if (g_plc_netcfg.lan.ioExp == 1)  //TCP/IP协议
    {
...
bool kalyke_can_arp(struct netif *netif)
{
    ...
    if (netif == (&fsl_netif1))
    {
        if (g_plc_netcfg.lan.ioExp == 2 || g_plc_netcfg.lan.ioExp == 0)
        {
            return false;
        }
    }
...
void kalyke_internet_task(void *arg)
{
    ...
    while(1)
    {
        ...
        {
            LOGI("internet_task", "ENET: PHY link down!");
            if (gWanOK == true)
            {
                if (mqtt_client)
                {
        ...
        {
            LOGI("internet_task", "ENET2: PHY link down!...");
            if (gLanOK == true)
            {
