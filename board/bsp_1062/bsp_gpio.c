...
pFuncRefreshIO gGPIORefreshIO = NULL;
...
void GPIO1_Combined_16_31_IRQHandler(void)
{
    ...
    else if (X5_PIN_MASK & GPIO_PortGetInterruptFlags(X5_GPIO))
    {
...
void GPIO2_Combined_0_15_IRQHandler(void)
{
    ...
    else if (MCU_CFG6_MASK & GPIO_PortGetInterruptFlags(MCU_CFG6_GPIO))
    {
        ...
        {
            GPIO_PortClearInterruptFlags(MCU_CFG6_GPIO, MCU_CFG6_MASK);
        }
    }
    SDK_ISR_EXIT_BARRIER;
}
...
static void bsp_gpio_other_init(void)
{
    ...
    NVIC_SetPriority(X0_IRQ, configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY);
    NVIC_SetPriority(X1_IRQ, configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY);
    NVIC_SetPriority(X2_IRQ, configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY);
    NVIC_SetPriority(X3_IRQ, configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY);
    EnableIRQ(X0_IRQ);
    EnableIRQ(X1_IRQ);
    //EnableIRQ(X2_IRQ); // Already EnableIRQ(MCU_CFG6_IRQ);
    EnableIRQ(X3_IRQ);
    EnableIRQ(X4_IRQ);
}
...
void bsp_gpio_init(void)
{
    ...
    /* Init input switch GPIO. */
#ifdef X0_AS_LED
    GPIO_PinInit(X0_GPIO, X0_GPIO_PIN, &sw_config_Y);
#else
